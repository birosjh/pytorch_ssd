...
class SSDLoss(nn.Module):
    ...
    def calculate_confidence_loss(self, pred_confidences, target_confidences, positive_matches, num_matched):

        positive_loss = self.calculate_positives_confidence_loss(
            pred_confidences[positive_matches],
            target_confidences[positive_matches]
        )
    ...
    def calculate_positives_confidence_loss(self, matched_confidences, matched_target_confidences):
    ...
    def calculate_negatives_confidence_loss(self, unmatched_confidences, num_matched):
        ...
        unsorted_negative_confidence_losses = self.confidence_loss(
            ...
            torch.zeros(
                unmatched_confidences.shape[0],
                dtype=int
            ).to(self.device)
        )
        negative_confidence_losses = unsorted_negative_confidence_losses.sort(descending=True).values

        return negative_confidence_losses[0:3 * num_matched].sum()

    
    def calculate_localization_loss(self, matched_localizations, matched_target_localizations):
    ...
    def forward(self, confidences, localizations, targets):
        ...
        for conf, loc, target in zip(confidences, localizations, targets):
            ...
            loss, conf_loss, loc_loss, hit_count = self.forward_per_item(conf, loc, target)
